...
- 2026-05-25 (Task #82): hook J06→J08 implementado — endpoint `POST /api/contratante/medicoes/[id]/aprovar` envolve `UPDATE medicoes SET status='aprovada'` + `criarLancamentoFromMedicao()` + `recomputeObraProgresso()` numa `db.transaction()`; falha em qualquer ponto reverte tudo (sem medição aprovada sem fatura sem progresso recalculado). `recomputeObraProgresso(obraId, tx?)` ganhou param opcional pra rodar com o mesmo client da transação. Audit gera 2 eventos: `medicoes.aprovar` (já existia, agora com `lancamentoId` no payload) + `financeiro.criar.from-medicao` (novo, ouvido por J13).
...
- 2026-05-25 (Task #82): invariante "toda aprovação gera fatura" virou **hard fail antes da transação**. Se a obra não tem `cliente_id` resolvível pra `userId` → 422 `OBRA_SEM_CONTRATANTE_USER` (nada persiste). Se `valorLancamento <= 0` (nem `medicao.valor` nem `obra.valorTotal` informados) → 422 `VALOR_INVALIDO`. Aprovação nunca passa sem lançamento — em todo caminho de sucesso, `lancamentoId` é garantido no payload do audit e na resposta.
