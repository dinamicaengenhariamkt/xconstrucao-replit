...
- 2026-07-16 (descoberto na J41): os E2E `admin lista threads…` e `abrir thread registra admin.chat.read…` passaram a falhar com **403 `PASSWORD_CHANGE_REQUIRED`** — não era bug do endpoint, e sim estado do banco: o admin seed foi promovido a `superadmin` e ganhou `must_change_password=true` (gate J22/J30), que bloqueia toda rota autenticada. Causa raiz no **helper de teste**, não no código de produção. Fix: `/api/test/login-as` passou a neutralizar `must_change_password` do usuário-alvo em modo `E2E_TEST_AUTH=1` (mesma intenção do endpoint, que já pula os gates de senha/email para exercitar guards downstream) — [app/api/test/login-as/route.ts](../../app/api/test/login-as/route.ts). Sem alteração na lógica do painel admin.
