...
- 2026-07-19 (Task #206): Notificações admin in-app para eventos de assinatura (J11). Dispatcher `features/notificacoes/assinatura-admin-dispatcher.ts` — grava tipo `info` para todos admin/superadmin nos eventos: `checkout` (nova assinatura), `cancelamento`, `inadimplente` (payment_failed via webhook), `reativacao` (payment_succeeded de inadimplente). Chamado fire-and-forget pós-commit em `assinatura-service.ts`. Sem email (ops-only).
