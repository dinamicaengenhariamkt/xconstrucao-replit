...
- **`chat_mensagens.seq`** (Task #149): coluna `BIGINT GENERATED ALWAYS AS IDENTITY` — sequência monotônica de chegada, fonte de verdade da ordem cronológica (substitui o desempate por `id` UUID aleatório). Adicionada idempotente no bootstrap + índice `idx_chat_mensagens_thread_seq (thread_id, seq)`. A listagem e o cursor de paginação passam a ordenar por `seq`.
...
- [x] **Reconciliação por id real** (fix definitivo do race de sobreposição): `chat-store.ts` ganha `confirmLocalMessage(convId, localId, serverId)` que *marca* a otimista com o id do servidor em vez de removê-la; `use-send-message.ts` (ambas personas) chama `confirmLocalMessage(convId, localId, data.id)` no `onSuccess` (aproveitando o `SendResponse.id` antes ignorado) e mantém `clearLocalMessage` só no `onError`; `app/{empreiteiro,contratante}/chat/page.tsx` deduplicam o merge por `serverId` (`serverIds` Set); `features/shared/xchat/types/index.ts`: `Message` ganha `serverId?`. Resultado: enviar N mensagens em sequência não faz nenhuma sumir/piscar. _(Task #149)_
- [x] **Ordem cronológica estável por `seq`** (fix da ordenação — mensagens novas apareciam fora de posição): `chat_mensagens` ganha coluna `seq` (BIGINT identity, fonte de verdade da ordem) via `server/bootstrap-chat.ts` + índice `(thread_id, seq)`; `shared/db/schema.ts` reflete a coluna; `features/chat/service.ts` ordena/pagina por `seq` (`ORDER BY m.seq DESC`, keyset `m.seq < before.seq`); `features/chat/cursor.ts` passa a cursor por `seq`; `features/chat/dto.ts` + `features/shared/xchat/types/index.ts` propagam `seq?`; novo `features/shared/xchat/lib/sort-messages.ts` (`sortMessagesCanonical`) ordena o merge por `seq` (otimistas sem seq ficam no fim, ordem de append estável); `app/{empreiteiro,contratante}/chat/page.tsx` aplicam o sort ao merge. Substitui o desempate por UUID aleatório. _(Task #149)_
...
- 2026-07-17 (Task #149): **Ordem cronológica instável** — mensagens novas apareciam fora de posição (às vezes acima das antigas) e podiam reordenar entre refetches. **Causa-raiz:** não havia chave de ordenação confiável — o backend desempatava por `id` (`gen_random_uuid()`, **aleatório**) e o merge do cliente (`[...server, ...local]`) não tinha sort canônico; duas mensagens no mesmo instante ficavam em ordem arbitrária. **Solução (fonte de verdade no banco, aproveitando que não há produção):** coluna `chat_mensagens.seq` (`BIGINT GENERATED ALWAYS AS IDENTITY`) — ordem total monotônica de chegada; backend passa a ordenar/paginar por `seq` (listagem `ORDER BY m.seq DESC` + `.reverse()`; keyset `m.seq < before.seq`; cursor por `seq`); DTO/types propagam `seq?`; novo helper `sortMessagesCanonical` ordena o merge no cliente (otimistas sem `seq` ficam no fim até o refetch, preservando ordem de append). Corrige a ordem em todas as camadas (tela, paginação, "última mensagem", buscas futuras). O dedupe por `serverId` da correção anterior é preservado (não regride). Migration idempotente no `bootstrap-chat.ts`.
- 2026-07-16 (Task #149): A correção anterior do Item 11 (`clearLocalMessage` singular) foi **paliativa** — não resolveu a sobreposição/some de mensagem ao enviar em rápida sequência. **Causa-raiz real:** o `onSuccess` do envio *removia* a mensagem otimista imediatamente e dependia do refetch (`staleTime: 60s` + latência) para "trazê-la de volta"; entre a remoção e o refetch a mensagem sumia do estado. Com dois envios concorrentes (A, B) as janelas se sobrepunham (o refetch de A voltava antes de B commitar) e uma mensagem "comia" a outra. Agravantes: merge sem dedupe (`[...server, ...local]`) e o `SendResponse.id` (UUID real) devolvido pelo backend era **ignorado**. **Solução:** reconciliação por id real — novo `confirmLocalMessage(convId, localId, serverId)` no `chat-store.ts` *marca* a otimista com o id do servidor (não remove); os hooks passam a chamar isso no `onSuccess` (usando `data.id`); o merge nas páginas deduplica por `serverId`, descartando a otimista só quando a versão do servidor chega — sem flicker, sem duplicata, envios concorrentes independentes. `clearLocalMessage` segue só no `onError` (rollback).
