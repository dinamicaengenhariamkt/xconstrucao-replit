...
- [x] Remover mocks + flag `NEXT_PUBLIC_ENABLE_EMPREITEIRO_MOCK` deste módulo _(Task #85 — flag virou default OFF via `isMockEnabled()` helper; mocks ficam disponíveis só com opt-in explícito de dev)_
...
- 2026-05-26 (Task #93): `/api/obras` (role=empreiteiro) agora aceita `?na_minha_zona=true` (filtro server-side via mesma expressão SQL que calcula a flag `naMinhaZona`) e a ordenação default passa a ser `zonaMatch DESC, createdAt DESC` quando o perfil tem zona configurada — obras na zona sobem pro topo sem precisar do toggle. Payload do empreiteiro ganha `zonaConfigurada: boolean` no envelope (omitido para outras roles). Front `/empreiteiro/novas-obras` ganhou toggle `toggle-na-minha-zona` + dois empty-states: (a) `empty-state-zona-nao-configurada` com CTA para `/empreiteiro/configuracoes` quando o toggle está ligado e a zona está vazia; (b) `empty-state-zona-sem-resultados` com botão pra desligar o filtro + link "ampliar zona". Fecha o gap de Task #87 §13 sobre filtro/ordenação.
- 2026-05-26 (Task #85): Default global da flag `NEXT_PUBLIC_ENABLE_EMPREITEIRO_MOCK` invertido pra **OFF** em todos os módulos (21 callsites migrados pra `isMockEnabled()` em `features/shared/lib/mock-flag.ts`). `features/contratante/dashboard/constants.ts` deixou de hardcodar `true` (era bug — contratante via stats falsos mesmo em prod). `features/admin/disputas/constants.ts` corrigiu default invertido (`!== 'false'` → helper, default OFF). `.env.local` agora seta `=false` com comentário explicando opt-in pra dev offline. Mocks NÃO foram deletados — só param de ser consumidos por default; ligar a flag pra `true` reativa.
