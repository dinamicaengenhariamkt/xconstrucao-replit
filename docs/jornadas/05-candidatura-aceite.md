...
- 2026-05-25 (Task #66 — J05.B): Gap acima fechado. Tabela `candidatura_anexos` criada via `server/bootstrap-candidatura-anexos.ts`, novo kind `candidatura_anexo` (privado, 10MB) no pipeline R2 (`key-builder.ts` + `validation.ts`), endpoints `GET/POST /api/empreiteiro/candidaturas/[id]/anexos` + `DELETE …/[anexoId]` (rate-limited, audit `candidaturas.anexo.{create,delete}`, mutação travada quando `status≠pendente`). Listagens `GET /api/empreiteiro/candidaturas` e `GET /api/contratante/obras/[id]/candidaturas` embedam `anexos[]` com signed URLs em 1 query agregada (sem N+1). UI: `aplicar/page.tsx` agora faz upload real via `useUpload({kind:'candidatura_anexo'})` antes do submit, com status enviando/pronto/erro e cleanup do `user_files` no remove pré-submit; após criar a candidatura faz N POSTs para vincular. `CandidaturasCard` (contratante) renderiza bloco "Anexos (N)" no `PropostaModal` com download em nova aba via signed URL. `/empreiteiro/minhas-candidaturas` mostra chips 📎 por candidatura. Chave R2 é user-scoped (`private/empreiteiro/{userId}/candidatura-anexos/…`) — vinculação à candidatura é via DB, não via path, pra ficar compatível com o `validateKeyForOwner` existente.
- 2026-07-19 (Task #207): Erro HTTP 402 `LIMITE_PLANO` em `POST /api/empreiteiro/candidaturas` agora exibe `PlanoUpsellDialog` (componente compartilhado em `features/planos/ui/`) em vez de `alert()` — CTA leva para `/empreiteiro/planos`. Demais erros passaram a usar `useToast` (sem alert).
