...
> **Itens 21–22 — Ajustes finos rodada 2 (reportado em uso real, 2026-07-14).** (21) legibilidade do bloco Endereço no cadastro; (22) investigação do relato "obra publicada não aparece p/ empreiteiro".

### Item 21 — Legibilidade do bloco "Endereço" no cadastro
- [x] Com os campos novos (Logradouro/Número/Complemento, Item 18), o grid `md:grid-cols-6` ficou desalinhado: Número (`col-span-2`) órfão numa linha e Complemento (`col-span-6`) quebrando sozinho. Reorganizado para linhas que somam 6: **CEP(2)+Logradouro(4) / Número(2)+Complemento(4) / Cidade(4)+UF(2) / Acessibilidade(6)**. Removido o `max-w-[10rem]` do `CepInput` para o campo preencher a coluna (consistente com os demais). Em [app/contratante/nova-obra/page.tsx](../../app/contratante/nova-obra/page.tsx). `data-testid` preservados.

### Item 22 — "Obra publicada não aparece p/ empreiteiro" — investigado (backend OK) + hardening do erro silencioso
- [x] **Investigação (reprodução ao vivo como o empreiteiro no dev):** a `GET /api/obras` **retorna a obra corretamente** (publicada + aprovada + sem empreiteira + sem auto-candidatura passam; `dbToNovaObra` mapeia sem erro; os cards renderizam). **O backend/filtro NÃO estava quebrado** — o "vazio" era estado transitório do navegador (confirmado pelo cliente: "agora mostrou"). Provável gatilho: toggle "Só na minha zona" ligado com zona vazia → 0 resultados + empty-state "Configure sua zona".
- [x] **Bug real encontrado e corrigido:** o endpoint `GET /api/empreiteiro/perfil-status` (consumido por `usePerfilStatus`) **nunca existiu** → 404 em todo carregamento da tela "Novas Obras", poluindo o console e disparando retries a cada foco. Criada a rota [app/api/empreiteiro/perfil-status/route.ts](../../app/api/empreiteiro/perfil-status/route.ts) retornando `PerfilStatus` válido (`isBlocked:false` — não há sistema de bloqueio no servidor; `completionPercentage`/`pendencias` calculados do perfil real).
- [x] **Hardening do erro silencioso (a causa da confusão):** (a) [app/empreiteiro/novas-obras/page.tsx](../../app/empreiteiro/novas-obras/page.tsx) passa a ler `isError` e mostra um **estado de erro com "Tentar novamente"** em vez de lista vazia silenciosa; (b) [novas-obras-service.ts](../../features/empreiteiro/novas-obras/api/novas-obras-service.ts) faz o `.map(dbToNovaObra)` **resiliente** (try/catch por row → descarta só a row ruim + `console.error`), para uma row malformada nunca zerar a lista inteira.
...
21. Endereço legível (item 21): em `/contratante/nova-obra`, o bloco Endereço mostra linhas alinhadas — CEP+Logradouro, depois Número+Complemento, depois Cidade+UF — sem campos órfãos; o CEP preenche sua coluna. Publicar continua exigindo número.
22. Empreiteiro (item 22): `GET /api/empreiteiro/perfil-status` retorna 200 (não mais 404). Obra publicada+aprovada aparece em "Novas Obras" (confirmado). Se a API de obras falhar → a tela mostra **estado de erro com "Tentar novamente"**, não lista vazia silenciosa; uma row malformada não zera a lista inteira.
