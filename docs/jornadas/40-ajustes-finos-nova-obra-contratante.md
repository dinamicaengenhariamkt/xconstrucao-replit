...
>
> Itens 1–8 concluídos. Itens 10–16 **implementados e verificados** (adapter/GET agregam dados reais, vazamentos financeiros corrigidos, itens 14/15 cobertos por E2E que passa em API-shape). Item 9 (upload R2): **código concluído, mas validação funcional BLOQUEADA** — a credencial R2 no ambiente ainda não autentica (401 em toda operação, inclusive ListBuckets). Ver Item 9 e Gaps (2026-07-13, validação).
...
> **Pendente — aguardando acesso ao bucket R2.** Cliente reportou erro ao anexar foto/documento na tela `/empreiteiro/novas-obras/[id]/aplicar` (card "Documentos Adicionais", kind `candidatura_anexo`). Fluxo: presign → **PUT direto do browser ao R2** → commit ([features/shared/hooks/use-uploads.ts](../../features/shared/hooks/use-uploads.ts)). A mensagem vem do `xhr.onerror` do PUT (`use-uploads.ts:65`).
>
> **Diagnóstico (comprovado por teste server-side contra o R2):** a **credencial R2 está inválida** — TODA operação retorna `SignatureDoesNotMatch` (403), inclusive `ListBuckets` (nível de conta) e com assinatura SigV4 **manual** (descarta CORS, checksum do SDK, path-style, endpoint/bucket). O par `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY` não corresponde na conta Cloudflare (token revogado/expirado ou secret copiado errado). O print enviado pelo cliente traz os mesmos valores do ambiente → confirma que precisam ser **regerados**. `R2_ACCOUNT_ID`/`R2_BUCKET`/`R2_S3_ENDPOINT`/`R2_PUBLIC_BASE_URL` estão corretos. O usuário vê "Falha de rede" em vez de "403" porque, sem CORS, o browser não lê a resposta 403 do PUT cross-origin.

- [ ] **Credencial (bloqueante) — AINDA NÃO VALIDADA.** As secrets foram trocadas pelo cliente, mas teste server-side contra o bucket real (2026-07-13) retornou **401 Unauthorized** em TODA operação, inclusive `ListBuckets` (nível de conta). Causas prováveis, em ordem: (a) o processo em execução carrega a credencial **antiga** — o `S3Client` é cacheado em módulo (`shared/lib/storage/r2.ts:13-14`), então trocar a secret **exige reiniciar/redeployar** o app; (b) o R2 API Token pode estar desabilitado ou sem permissão **Object Read & Write** no bucket. Bucket no Cloudflare segue **0 B / 0 operations** → nenhum upload real chegou. **Validação real só pelo app do Replit reiniciado** (não dá para validar por print — o campo do secret é truncado; nem pelo presign — é matemática local, sempre "OK" sem tocar a rede). _(reaberto na validação de 2026-07-13)_
