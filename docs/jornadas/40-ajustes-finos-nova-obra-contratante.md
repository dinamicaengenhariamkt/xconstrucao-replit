...
>
> Itens 1–8 concluídos. Itens 9–16 **pendentes** (upload R2 + auditoria ponta-a-ponta do fluxo da obra: hardcodes no detalhe da obra e vazamentos financeiros). Bloqueio principal: credencial R2 (item 9).
...
- [x] **Credencial (bloqueante):** secrets R2 atualizados (AccessKey + SecretKey novos), validados via `ListBuckets` → bucket `xconstrucao-app-files` retornado com 2xx. _(Task #96)_
