...
- **Bootstrap CLI**: `npx tsx scripts/bootstrap-superadmin.ts --email <e> --name <n> --password <p> [--force-reset-password]` (também aceita env `SUPERADMIN_EMAIL`/`NAME`/`PASSWORD`/`FORCE_RESET=YES` ou prompts interativos). Valida política, garante invariante de pelo menos 1 super admin ativo, e grava `audit_logs` (`action="cli.bootstrap-superadmin"`, ip="cli").
