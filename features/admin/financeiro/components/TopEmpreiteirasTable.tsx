...
import type { TopEmpreiteira } from '../types';
import { formatCurrency } from '../utils';
import { cn } from '@shared/lib/utils';

interface TopEmpreiteirasTableProps {
    ...
    <Card className="border-border-light dark:border-gray-800 overflow-hidden">
      <CardHeader className="border-b border-gray-100 dark:border-gray-800">
        <CardTitle className="text-base font-bold text-gray-900 dark:text-gray-100">
          Top empreiteiras por volume
        </CardTitle>
        <CardDescription className="text-xs text-gray-500">
          Maiores executoras da plataforma
        </CardDescription>
      </CardHeader>
      <CardContent className="p-0">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {empreiteiras.map((emp, idx) => {
                const isLast = idx === empreiteiras.length - 1;
                return (
                  <tr
                    key={emp.id}
                    className={cn(
