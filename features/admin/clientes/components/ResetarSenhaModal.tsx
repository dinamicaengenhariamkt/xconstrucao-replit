...
import { useResetarSenhaCliente } from '../hooks/use-clientes';
...
export function ResetarSenhaModal({
  ...
  const { mutateAsync, isPending } = useResetarSenhaCliente();
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        ...
        data-testid="modal-resetar-senha"
      >
        {/* Header */}
          ...
          {enviado ? (
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  O link de redefinição de senha foi enviado para{' '}
                  <span className="font-semibold text-gray-700 dark:text-gray-300">{emailCliente}</span>.
                </p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-4">
              <p className="text-sm text-gray-600 dark:text-gray-300">
                Será enviado um link de redefinição de senha para o e-mail cadastrado de{' '}
                <span className="font-semibold text-gray-900 dark:text-white">{nomeCliente}</span>:
              </p>
              ...
              <p className="text-xs text-gray-400">
                O link expira em 24 horas. O cliente precisará acessar o e-mail para criar uma nova senha.
              </p>
        ...
        <DialogFooter className="p-6 border-t border-gray-100 dark:border-gray-800 shrink-0 flex flex-row justify-end gap-3">
          {enviado ? (
            <Button onClick={handleClose} data-testid="button-fechar-resetar-senha">
              Fechar
            </Button>
          ) : (
            <>
              <Button
                ...
                onClick={handleClose}
                disabled={isPending}
                data-testid="button-cancelar-resetar-senha"
              >
                Cancelar
              </Button>
              <Button
                ...
                data-testid="button-confirmar-resetar-senha"
              >
