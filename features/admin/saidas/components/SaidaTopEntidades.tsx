...
import type { SaidaTopItem } from '../types';
...
function TopTable({ title, description, entityLabel, valueLabel, items, testIdPrefix }: TopTableProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (valorMinNum !== undefined) {
      result = result.filter((i) => i.totalSaidas >= valorMinNum);
    }
    if (valorMaxNum !== undefined) {
      result = result.filter((i) => i.totalSaidas <= valorMaxNum);
    }
        ...
        {advancedActiveCount > 0 && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(valorMinNum !== undefined || valorMaxNum !== undefined) && (
              <ActiveFilterChip
                label={`${valueLabel}: ${formatRange(valorMin, valorMax, 'R$ ')}`}
                onRemove={() => {
                  setValorMin('');
                  setValorMax('');
                }}
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50/50 dark:bg-gray-800/50">
                ...
                <TableHead className="text-xs font-bold uppercase tracking-wider">{valueLabel}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered.map((item) => (
...
interface SaidaTopEntidadesProps {
  empreiteiras: SaidaTopItem[];
  clientes: SaidaTopItem[];
}

export function SaidaTopEntidades({ empreiteiras, clientes }: SaidaTopEntidadesProps) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <TopTable
        title="Top empreiteiras por pagamentos"
        description="Empreiteiras com maior volume de pagamentos no período"
        entityLabel="Empreiteira"
        valueLabel="Total pago"
        items={empreiteiras}
        testIdPrefix="top-empreiteiras-saidas"
      />
      <TopTable
        title="Top clientes por reembolsos"
        description="Clientes com maior volume de reembolsos no período"
        entityLabel="Cliente"
        valueLabel="Total reembolsado"
        items={clientes}
        testIdPrefix="top-clientes-saidas"
      />
    </div>
  );
}
