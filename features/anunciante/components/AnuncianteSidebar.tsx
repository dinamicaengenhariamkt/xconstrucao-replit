...
import { ANUNCIANTE_NAV_ITEMS, ANUNCIANTE_BOTTOM_NAV_ITEMS } from '../constants';

export function AnuncianteSidebar() {
  ...
  const isActive = (url: string): boolean => {
    if (url === '/anunciante/dashboard') return pathname === url;
  ...
  return (
    <Sidebar className="bg-[#FAFAFA] dark:bg-background-dark">
      ...
      <SidebarFooter className="p-4">
        <SidebarMenu>
          {ANUNCIANTE_BOTTOM_NAV_ITEMS.map((item) => (
            <SidebarMenuItem key={item.title}>
