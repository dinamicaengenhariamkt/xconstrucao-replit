...
import type { ContratanteConversation, ContratanteMessage, ContratanteMessageAttachment } from '../types';

interface ContratanteChatStore {
  selectedConversationId: string | null;
  searchQuery: string;
  filterTab: 'all' | 'unread';
  localMessages: Record<string, ContratanteMessage[]>;
  ephemeralConversations: ContratanteConversation[];
  setSelectedConversation: (id: string) => void;
  setSearchQuery: (query: string) => void;
  setFilterTab: (tab: 'all' | 'unread') => void;
  sendMessage: (conversationId: string, content: string, attachment?: ContratanteMessageAttachment) => void;
  addEphemeralConversation: (conv: ContratanteConversation) => void;
}
...
export const useContratanteChatStore = create<ContratanteChatStore>((set) => ({
  ...
  sendMessage: (conversationId, content, attachment) => {
    ...
    const newMessage: ContratanteMessage = {
