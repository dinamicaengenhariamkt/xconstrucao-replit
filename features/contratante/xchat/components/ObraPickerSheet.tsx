...
import type { ContratanteObraPickerItem, ContratanteObraRefAttachment } from '../types';
...
interface ObraPickerSheetProps {
  ...
  onSelect: (attachment: ContratanteObraRefAttachment) => void;
...
export function ObraPickerSheet({ obras, onSelect, children }: ObraPickerSheetProps) {
  ...
  function handleSelect(obra: ContratanteObraPickerItem) {
    onSelect({
      type: 'obra_ref',
      obraId: obra.id,
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    <span className="text-[10px] text-gray-400 flex-shrink-0">
                      {obra.progresso}% · {STATUS_LABEL[obra.status] ?? obra.status}
                    </span>
                  </div>
                </div>
              </button>
