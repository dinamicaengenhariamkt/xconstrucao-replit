...
import {
  ...
  CONTRATANTE_FAQ_CATEGORY_META,
  CONTRATANTE_FAQ_CATEGORY_ICONS,
  CONTRATANTE_FAQ_SUBTITLE,
} from '@features/contratante/faq/constants';
import { RiQuestionLine } from 'react-icons/ri';

const CATEGORY_OPTIONS = Object.entries(CONTRATANTE_FAQ_CATEGORIES).map(
...
export default function ContratanteFAQPage() {
  ...
  return (
    <div className="p-10 flex flex-col gap-10" data-testid="faq-contratante-page">
      <FAQHeroSearch
        title="Perguntas Frequentes"
        subtitle={CONTRATANTE_FAQ_SUBTITLE}
        searchQuery={searchQuery}
        onSearchChange={setSearchQuery}
        searchPlaceholder="Buscar por palavra-chave (obras, empreiteiros, pagamentos...)"
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        >
          <MultiSelectDropdown<string>
            ...
            testIdPrefix="filter-faq-contratante-categoria"
          />
        </AdvancedFiltersPopover>
        ...
        {advancedActiveCount > 0 && (
          <div className="flex items-center gap-2 flex-wrap justify-center">
            {categorySelected.map((c) => (
              <ActiveFilterChip
                key={c}
                label={`Categoria: ${CONTRATANTE_FAQ_CATEGORIES[c] ?? c}`}
                onRemove={() =>
                  setCategorySelected(categorySelected.filter((x) => x !== c))
                }
                testId={`active-chip-faq-contratante-categoria-${c}`}
              />
            ))}
          </div>
        )}
      ...
      <div className="space-y-8">
        {Object.entries(groupedItems).map(([category, categoryItems]) => {
          const meta = CONTRATANTE_FAQ_CATEGORY_META[category];
          const Icon = CONTRATANTE_FAQ_CATEGORY_ICONS[category] ?? RiQuestionLine;
          return (
            <div key={category}>
              <FAQSectionHeader
                label={CONTRATANTE_FAQ_CATEGORIES[category] || category}
                iconBg={meta?.iconBg}
                iconColor={meta?.iconColor}
                Icon={Icon}
              />
