...
import { useAdminEmpreiteira, useAdminEmpreiteiraObras } from '@features/admin/empreiteiras/hooks/use-empreiteiras';
import {
  RiArrowLeftLine,
  RiBriefcaseLine,
  RiUserLine,
...
export default function AdminEmpreiteiraObrasPage() {
  const params = useParams();
  const id = params.id as string;
  const { data: empreiteira } = useAdminEmpreiteira(id);
  const { data: obras, isLoading } = useAdminEmpreiteiraObras(id);
  ...
  return (
    <div className="p-6 md:p-10 space-y-8" data-testid="admin-empreiteira-obras-page">
      <Link href={`/admin/empreiteiras/${id}`} data-testid="link-back-empreiteira-detail">
        <Button variant="ghost" size="sm" data-testid="button-back-empreiteira-detail">
      ...
      <div>
        <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">
          Obras {empreiteira ? `de ${empreiteira.nomeFantasia}` : ''}
        </h1>
      ...
      <div className="flex gap-2 border-b border-gray-200 dark:border-gray-800">
        <Link href={`/admin/empreiteiras/${id}`} data-testid="tab-cadastro-obras">
          <Button variant="ghost" className="rounded-none text-muted-foreground" data-testid="button-tab-cadastro-obras">
            Cadastro
          </Button>
        </Link>
        <Link href={`/admin/empreiteiras/${id}/obras`} data-testid="tab-obras-active">
          <Button
            variant="ghost"
      ...
      {obras && obras.length > 0 ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Esta empreiteira ainda não possui obras cadastradas.</p>
        </div>
      )}
    </div>
  );
