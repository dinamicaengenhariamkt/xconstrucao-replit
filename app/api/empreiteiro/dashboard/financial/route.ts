...
import { getEmpreiteiroFinancial } from '@features/empreiteiro/dashboard/api/dashboard-server';
...
export async function GET(request: NextRequest) {
  ...
  const financial = await getEmpreiteiroFinancial(guard.user.id, periodo);
  const r = NextResponse.json(financial);
  setNoCacheHeaders(r);
  return r;
}
